...
- `Por que elegirnos` debe mantenerse como una escena cinematografica oscura con deck de tarjetas glass verticales, superpuestas e inclinadas de forma alternada. La seccion usa titulo display, eyebrow mono, lead i18n desde `siteContent`, iconos Phosphor y scroll horizontal responsive; no volver al grid plano salvo decision explicita.
...
- 2026-05-20: Se redisenio `Por que elegirnos` como deck cinematografico de tarjetas verticales glass superpuestas con inclinacion alternada, encabezado/lead i18n y scroll horizontal responsive, preservando el ambiente oscuro premium de la web. Verificado con `npm run lint`, `npm run typecheck`, React Doctor y QA visual en Browser/Playwright; no se ejecuto build por regla del repo.
