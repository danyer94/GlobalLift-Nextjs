...
- `Por que elegirnos` debe mantenerse como una escena cinematografica oscura con deck de tarjetas glass verticales, superpuestas e inclinadas de forma alternada en desktop. En mobile (mismo rango que el header `md:hidden`, hasta 767px) NO usar el deck desktop comprimido ni carrusel horizontal: usar una pila vertical de tarjetas compactas, legibles, con inclinacion sutil alternada y sin overflow lateral. La seccion usa titulo display, eyebrow mono, lead i18n desde `siteContent` e iconos Phosphor; no volver al grid plano salvo decision explicita.
