...
- `ProductGallery` debe mantener un carrusel 3D tipo coverflow premium: imagen activa frontal, tarjetas laterales/fondo en perspectiva 3D con profundidad, blur/brightness por distancia, controles glass accesibles, dots de progreso, visor modal y pausa de autoplay en hover/touch. No volver al carrusel plano con miniaturas lineales salvo decision explicita.
...
- 2026-05-20: `ProductGallery` se reconstruyo como carrusel 3D premium inspirado en el mockup generado: stage oscuro cinematografico, cards laterales en perspectiva, card activa frontal, controles glass, dots, contador y visor modal conservado. Verificado con `npm run lint`, `npm run typecheck`, React Doctor y QA visual Playwright desktop/mobile; no se ejecuto build por regla del repo.
